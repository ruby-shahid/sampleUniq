class Document < ApplicationRecord
  ...
  def get_dup_attributes
    raw_object_one = Roo::CSV.new(file_one.url).column(1).compact.uniq rescue nil
    raw_object_two = Roo::CSV.new(file_two.url).column(1).compact.uniq rescue nil
    raw_object_three = Roo::CSV.new(file_tree.url).column(1).compact.uniq rescue nil
    raw_object_four = Roo::CSV.new(file_four.url).column(1).compact.uniq  rescue nil
